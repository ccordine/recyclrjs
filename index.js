...
class GX {
  ...
  fixQueryString(url) {
    ...
    // If URL construction fails, fall back to original input
    __recyclrDebugLog(this, 'fixQueryString fallback', e);
    return url;
  }
...
return newUrl;
  ...
  try {
    ...
    let condition = null
    let html = await applyBusyState(this?.target || document, true);
    fetch(this.url, requestBody).then(response => {
